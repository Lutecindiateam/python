...
class Face_atten:
    ...
    def attendance(self, logout_name):
        ...
        cursor.execute("SELECT * FROM logout_atten WHERE logout_name = %s AND logout_date = %s", (logout_name, logout_date))
        existing_entry = cursor.fetchone()

        if existing_entry:
            print(f"{logout_name} is already marked as present for {logout_date}")
        else:
            cursor.execute("INSERT INTO logout_atten(logout_name, logout_time, logout_date) VALUES ( %s, %s, %s)", (logout_name, logout_time, logout_date))
            conn.commit()
            print(f"{logout_name} marked as present for {logout_date} at {logout_time}")
    ...
    def process(self, stream):
        ...
        if self.get_face_database():
            while stream.isOpened():
                ...
                self.update_fps()
                cv2.namedWindow("logout_camera", 1)
                cv2.imshow("logout_camera", img_rd)
